...
const fs = require('fs');
const ticketsApi = require('./tickets.api');
const azureSearch = require('./azuresearch.api');
...
// Initiate bot and fallback if doesn't match any of dialogs
      ...
      if (result.response) {
        ...
        client.post('/api/tickets', data, (err, request, response, ticketId) => {
          if (err || ticketId == -1) {
            ...
            // typing response while processing demanding tasks or getting resources
            session.sendTyping();
            setTimeout(function () {
    ...
    if (args.result.value.length > 0) {
      ...
      args.result.value.forEach((faq, i) => {
        msg.addAttachment(
          ...
          .buttons([{
            title: 'More details',
            value: `show me the article ${faq.title}`,
            type: 'postBack'
          }])
        );
      });
...
// Luis recognizer to interpret user intents
...
bot.dialog('firstRun', function (session) {
  ...
  session.send("Hi <firstRun>...").endDialog();
}).triggerAction({
...
bot.on('conversationUpdate', function (message) {
  if (message.membersAdded && message.membersAdded.length > 0) {
    ...
    if (message.membersAdded[0].id !== 'default-bot') {
      ...
      var txt = isGroup ? "Hello everyone!" : "Welcome message!";
